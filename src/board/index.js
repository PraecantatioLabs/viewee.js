...
import EagleXMLParser from './parser-eagle-xml';
...
export default class ViewEEBoard {
	...
	constructor () {
		...
		// this is a layers as seen in editor
		this.layersCAD = [];

		// this is a manufacturing layers, like gerber
		this.layersCAM = [];
	}
	...
	static fromData (data, options) {
		...
		const timerLabel = `parsing using ${parser.name}`;
	...
	calculateBounds () {
		...
		// Plain elements
		Object.keys (this.plainWires).forEach (layerKey => {
			const lines = this.plainWires[layerKey];
